...
function install_gitpod() {
  ...
  yq e -i ".objectStorage.azure.certificate.kind = \"secret\"" "${CONFIG_FILE}"
  yq e -i ".objectStorage.azure.certificate.name = \"${SECRET_STORAGE}\"" "${CONFIG_FILE}"
  yq e -i '.workspace.runtime.containerdRuntimeDir = "/run/containerd/io.containerd.runtime.v1.linux/k8s.io"' "${CONFIG_FILE}"
